...
class _StudentAttendanceHistoryScreenState extends State<StudentAttendanceHistoryScreen> {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<AttendanceReportViewModel>().loadStudentHistory(widget.studentId);
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<AttendanceReportViewModel>(
        builder: (context, vm, child) {
          if (vm.isLoading) {
            return const Center(child: CircularProgressIndicator(color: AppColors.primary));
          }
          ...
          if (vm.studentHistory.isEmpty) {
            return Center(child: Text("No history available.", style: AppTypography.body1.copyWith(color: AppColors.grey5E)));
          }
                    ...
                    children: [
                      Column(
                        ...
                        children: [
                          ...
                          Text(DateFormat('EEEE').format(date), style: AppTypography.caption.copyWith(color: AppColors.grey5E)),
                          AppSpacing.h2,
